...
const GridDrowned=({items})=>{
    ...
    const resetAnimation=(animate=true)=>{
        if(!rendered.current && animate){
            drownedAnimation()
            rendered.current=true
        }
        if(!animating.current || !animate){
            ...
            <StyledGridContainer  sx={{p:0}} container spacing={{xs:.7, sm:1, md:1.5}}>
                {widths.map((width, index)=>{
                    ...
                    {!rightEdge?<StyledVerticalBar  className="vertical-bar"/>:<div className="vertical-bar-right-edge"></div>}                   
                        ...
                        {index<items.length?
                            <><img src={items[index].image.default} alt={items[index].name}/><Typography variant="subtitle1" component="span">{items[index].name}</Typography></>:
